...
V(dag)[is_leaf]
leaves_labels <- get.vertex.attribute(dag, "label",  V(dag)[is_leaf])
leaves_types <- get.vertex.attribute(dag, "type",  V(dag)[is_leaf])
...
search_res <- bfs(g, root = 1L, neimode = "out", callback = cbf)
search_res
```
